...
* 问题：网盘不支持 302 直连，不在家观看时受家宽上传速度限制。  
  alist webdav 挂载体验不好，无法通过 strm 里填挂载的地址，并启用路径转换解决。
* 解决方案：本机再运行一个 alist，同时启用直接播放 strm 内的地址。最后由脚本将 alist 的地址重定向到本机。
* 填写位置：`.ini` > `[dev]`
  ```
    # 替换播放器收到的播放地址，限 mpv pot 使用，其他播放器有问题不要反馈。
    # 注意配置，带 alist 端口，不然会把 emby 自身的播放地址变成本地，导致无法播放。
    # 还可能影响 stream_redirect 等，谨慎配置，保证唯一性。
    media_path_replace = http://192.168.1.1:5244, http://127.0.0.1:5244
  ```
